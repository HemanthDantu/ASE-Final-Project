...
public class SearchRecpie extends AppCompatActivity {
    ...
    private ArrayList<Bitmap> RandomBitmaps;
        ...
        btnSearch.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(SearchText.getText().toString()!="")
                {
                   ...
                   // mongoDB.ListDocuments("userdetails");
                }
                else
                {
    ...
    private class DownloadImages extends  AsyncTask <String,Integer,Double>
    {
        ...
        protected void onPostExecute(Double result)
        {
            try
            {
                Intent intent = new Intent(SearchRecpie.this, Login.class);
        ...
        protected void onPostExecute(Double result)
        {
            try
            {
                if(RandomRecipe!=null)
                {
                    ...
                    new DownloadRandomImages().execute();
    ...
    private class DownloadRandomImages extends  AsyncTask <String,Integer,Double>
    {
        ...
        private void getData() {
            ...
            try
            {
                for(int i=0;i<RandompairIamges.size();i++)
                {
                    String URI = RandompairIamges.get(i);
                    Bitmap bitmap = BitmapFactory.decodeStream((InputStream) new URL(URI).getContent());
                    RandomBitmaps.add(bitmap);
                }
            }catch (Exception ex)
            {
                ex.printStackTrace();
        ...
        protected void onPostExecute(Double result)
        {
            try
            {
                Imageview1.setImageBitmap(RandomBitmaps.get(0));
                Imageview2.setImageBitmap(RandomBitmaps.get(1));
                Imageview3.setImageBitmap(RandomBitmaps.get(2));
